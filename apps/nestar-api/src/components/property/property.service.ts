...
export class PropertyService {
    ...
    private likeServie: LikeService,){}
                                ...
                                $facet: {
                                    list: [{ $skip: (input.page - 1) * input.limit }, 
                                        { $limit: input.limit },
                                         //meLiked
                                         lookupMember,
                                         {$unwind: '$memberData'} //[memberDate] => memberData 
    ...
    propertyStatus: propertyStatus ?? {$ne: PropertyStatus.DELETE}}
      ...
      if(!target) throw new InternalServerErrorException(Message.NO_DATA_FOUND)
        ...
        //Like toggle
        const modifier: number = await this.likeServie.toggleLike(input)
                ...
                const result = await this.propertyModel.aggregate([
                    ...
                    { $sort: sort },
                    {
                        $facet: {
                            ...
                            lookupMember,//[memberValue]
                          {$unwind: '$memberData'}//memberValue
                        ],
                            metaCounter: [{ $count: "total" }]
                        }
                    }
