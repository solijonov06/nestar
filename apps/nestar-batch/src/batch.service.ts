...
export class BatchService {
  ...
  public async batchRollback(): Promise<void> {
    ...
    console.log("BbatchTopProperties executed");
    const properties: Property[] = await this.propertyModel
    .find({propertyStatus: PropertyStatus.ACTIVE,
      propertyRank: 0
    }
  ).exec();
  ...
  const promisedList = properties.map( async (ele: Property) => {
    ...
    .findByIdAndUpdate(_id, {propertyRank: rank
    })
  })
  ...
  const promisedList = agents.map( async (ele: Member) => {
    ...
    .findByIdAndUpdate(_id, {memberRank: rank
    })
  })
    await Promise.all(promisedList)
  }
